...
class _UserDashboardScreenState extends ConsumerState<UserDashboardScreen>
  ...
  Future<void> _refresh() async {
    await ref.read(dashboardProvider.notifier).loadData(background: true);
  ...
  Widget build(BuildContext context) {
    ...
    // Watch provider directly — no local state copy needed
    final dashboardState = ref.watch(dashboardProvider);
    final myPostsState = ref.watch(myPostsProvider);

    // Start animation when data arrives
    if (dashboardState.hasValue && !_animationController.isAnimating && _animationController.value == 0) {
      _animationController.forward();
    }
          ...
          body: Center(
            child: Column(
              ...
              children: [
                ...
                Text(
                  dashboardState.error.toString(),
                  style: TextStyle(color: isDark ? Colors.white70 : AppTheme.gray700),
    ...
    final data = dashboardState.valueOrNull ?? const DashboardData();
    final user = data.user;
    final stats = data.stats;
    final suggestions = data.suggestions;
    final completionRate = (stats['completionRate'] as num?)?.round() ?? 0;
    final recentActivities = stats['recentActivities'] is List
        ? stats['recentActivities'] as List
        : <dynamic>[];
                  ...
                  child: GridView.count(
                    ...
                    children: [
                      _buildStatCard(
                        ...
                        color: ((stats['resumeCount'] as num?)?.toInt() ?? 0) > 0
                            ? Colors.green
                            : Colors.orange,
                      ...
                      _buildStatCard(
                        title: 'Skills',
                        value: stats['skillsAssessed'] == true ? 'Done' : 'Not Done',
                        icon: Icons.psychology,
                        color: stats['skillsAssessed'] == true
                      ...
                      _buildStatCard(
                        ...
                        icon: Icons.send,
                        color: ((stats['appliedCount'] as num?)?.toInt() ?? 0) > 0
                            ? Colors.deepOrange
                            : Colors.grey,
  ...
  Widget _buildProfileCompletionCard(int completionRate, {User? user, Map<String, dynamic>? socialStats}) {
    final isDark = Theme.of(context).brightness == Brightness.dark;
    return TweenAnimationBuilder<double>(
                              ...
                              image:
                                  ...
                                  ? DecorationImage(
                                      image: CachedNetworkImageProvider(
                                        ImageHelper.getImageUrl(user.profilePictureUrl) ?? '',
                                      ),
                                      fit: BoxFit.cover,
                                    )
                                  : null,
  ...
  Widget _buildProfileAvatarFallback() {
    final user = ref.read(dashboardProvider).valueOrNull?.user;
    return Center(
      child: Text(
  ...
  void _showAllActivities(BuildContext context) {
    final isDark = Theme.of(context).brightness == Brightness.dark;
    final currentStats = ref.read(dashboardProvider).valueOrNull?.stats ?? {};
    final activities = currentStats['recentActivities'] is List
        ? currentStats['recentActivities'] as List
        : <dynamic>[];
