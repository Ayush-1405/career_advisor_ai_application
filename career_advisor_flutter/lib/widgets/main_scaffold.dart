...
class _MainScaffoldState extends ConsumerState<MainScaffold>
  ...
  void dispose() {
    ...
    ref.read(autoRefreshServiceProvider).stop();
    super.dispose();
  }
  ...
  int _totalUnread(AsyncValue? state) {
    if (state == null || !state.hasValue) return 0;
    final list = state.value as List?;
    if (list == null) return 0;
    return list.fold<int>(0, (total, room) {
      final unread = room.unreadCount;
      return total + (unread is int ? unread : 0);
    });
  }
  ...
  Widget build(BuildContext context) {
    ...
    ref.listen(myChatsProvider, (previous, next) {
      ...
      if (nextUnread > prevUnread && location != '/chat') {
    ...
    return PopScope(
      ...
      onPopInvokedWithResult: (didPop, result) {
        if (didPop) return;
        if (location != '/feed') context.go('/feed');
      },
      child: Scaffold(
        body: widget.child,
